                        ...
                        >
                            <template #default="scoped">
                                ...
                                <el-button
                                    type="danger"
                                    text
                                    @click.stop="handleRemove(scoped.row)"
    ...
    >
        <el-form ref="formRef" :model="formData" label-width="110px" :rules="rules">
            ...
            <el-form-item label="详细地址" prop="address">
                <el-input v-model="formData.address" placeholder="请输入详细地址" type="textarea" clearable />
            </el-form-item>
        </el-form>
        <template #footer>
...
<script setup>
import { nextTick, onMounted, reactive, toRefs, getCurrentInstance } from 'vue';
...
watch(
    () => props.modelValue,
    async (newVal) => {
        currentObject.value = cacheData['CrmCustomerArea'];
        let ids = [];
...
const submitForm = async () => {
    proxy.$refs['formRef'].validate(async (valid) => {
        if (valid) {
            const form = {
                ...formData.value,
                areaCode: formData.value.areaCode.join(','),
                areaName: findRegionName(formData.value.areaCode, areaOptions.value).join(','),
            };
