<template>
  <div class="search-group">
    ...
    <el-input
      ...
      :placeholder="
        config.paramType[paramKey]?.placeholder ? config.paramType[paramKey]?.placeholder : '请输入'
      "
    ...
    <el-select
      ...
      v-model="queryParams[paramKey]"
      :placeholder="
        config.paramType[paramKey]?.placeholder ? config.paramType[paramKey]?.placeholder : '请选择'
      "
    >
      <el-option
...
<script setup>
import { onBeforeMount, defineEmits, defineProps, reactive } from 'vue';
...
const paramsTag = computed(() =>
  Object.keys(queryParams.value).filter(key => !!queryParams.value[key])
);
