...
const { website, env, axios } = inject('globalConfig');
...
const uploadImgUrl = ref(env.VITE_APP_FLIEUPLOAD_CODE);
const headers = ref({
...
watch(
    ...
    (val) => {
        if (val) {
            ...
            fileList.value = list.map((item) => (typeof item === 'string' ? { name: item, url: item } : item));
        } else {
            fileList.value = [];
        }
    },
