...
<script setup>
import { inject, defineEmits, defineProps, reactive, toRefs, computed, watch, onMounted } from 'vue';

const { store, request, cacheData } = inject('globalConfig');
...
watch(
    ...
    (val) => {
        if (!Object.keys(cacheData).includes(val)) {
            console.warn(`[dc-select-remote] objectName "${val}" 不在全局 cacheData 中`);
        }
    },
    { immediate: true }
);
...
const getOptions = async () => {
    ...
    const res = await request.getQuery({
        url: currentObject.value.url,
        params,
    });
